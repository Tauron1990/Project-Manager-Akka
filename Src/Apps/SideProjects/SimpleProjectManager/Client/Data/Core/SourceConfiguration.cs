    ...
    where TState : class, new()
{
    ...
    private sealed class FetcherEffect<TToPatch> : IEffect
    {
        ...
        public Effect<ApplicationState> Build()
            => Effects.CreateEffect<ApplicationState>(BuildObserable(_patcher), true);
    ...
    private sealed class InitialProvider : IEffect
    {
        ...
        public Effect<ApplicationState> Build()
            => Effects.CreateEffect<ApplicationState>(_ => Observable.Return(_initial).Select(s => new SetNewState(_ => s)),  true);
    }
    ...
    private static On<ApplicationState> Setter(Guid id)
        => Reducers.On<SetNewState, ApplicationState>(
            (state, newState)
    ...
    private FetcherEffect<TToPatch> CreateState<TToPatch>(
        ...
        out Action<List<On<ApplicationState>>, List<IEffect>> config)
    {
    ...
    private Action<List<On<ApplicationState>>, List<IEffect>> CreateConfigurationForDatabase<TToPatch>(       
        Func<CancellationToken, Task<TToPatch>> fetcher, 
        Func<TState, TToPatch, TState> patcher)
        => (ons, list) =>
           {
    ...
    private Action<List<On<ApplicationState>>, List<IEffect>> CreateConfiguratorForServerOnly<TToPatch>(
        Func<CancellationToken, Task<TToPatch>> fetcher,
        Func<TState, TToPatch, TState> patcher)
        => (ons, list) =>
           {
    ...
    private IStateConfiguration<TState> CreateConfiguration(Action<List<On<ApplicationState>>, List<IEffect>> config)

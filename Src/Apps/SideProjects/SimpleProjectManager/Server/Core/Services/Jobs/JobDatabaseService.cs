...
public class JobDatabaseService : IJobDatabaseService, IDisposable
{
    ...
    public JobDatabaseService(InternalRepository database, CommandProcessor commandProcessor, DomainEventDispatcher dispatcher)
    {
    ...
    public virtual async Task<JobInfo[]> GetActiveJobs(CancellationToken token)
    {
        ...
        var result = await _projects.Find(Builders<ProjectProjection>.Filter.Not(filter))
           .Project(p => new JobInfo(p.Id, p.JobName, p.Deadline))
           .ToListAsync(token);

        return result.ToArray();
    }

    public async Task<SortOrder> GetSortOrder(ProjectId id, CancellationToken token)
    ...
    public async Task<ApiResult> ChangeOrder(SetSortOrder newOrder, CancellationToken token)
    {

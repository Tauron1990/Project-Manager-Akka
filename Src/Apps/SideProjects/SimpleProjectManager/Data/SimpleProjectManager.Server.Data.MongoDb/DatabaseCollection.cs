...
public class DatabaseCollection<TData> : IDatabaseCollection<TData>
{
    ...
    {
        var result = await _collection.UpdateOneAsync(GetRealFilter(filter), GetRealUpdater(updater), cancellationToken: cancellationToken);
    ...
    {
        var result = _collection.DeleteOne(GetRealFilter(filter));
    ...
    {
        var result = await _collection.DeleteOneAsync(GetRealFilter(filter), cancellationToken: token);

...
public sealed class IpSetup : ISetup
{
    ...
    {
        string newIp = await _clientInteraction.Ask(operationConfiguration.ServerIp.Value.EmptyToNull(), "Die IP Adresse zum Projekt Server?");
        int newPort = await _clientInteraction.Ask(operationConfiguration.ServerPort.Value.MinusOneToNull(), "Der Port zum Projekt Server (Stabdart: 4000)?");
        int newAkkaPort = await _clientInteraction.Ask(operationConfiguration.AkkaPort.Value.MinusOneToNull(), "Der Port zum Projekt Cluster (Standart: 4001)?");

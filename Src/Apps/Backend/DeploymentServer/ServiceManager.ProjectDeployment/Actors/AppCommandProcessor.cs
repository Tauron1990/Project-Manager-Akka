...
namespace ServiceManager.ProjectDeployment.Actors
{
    ...
    {
        public sealed record AppCommandProcessorState(IMongoCollection<AppData> Apps, GridFSBucket Files, RepositoryApi Repository, DataTransferManager DataTransfer,
            IMongoCollection<ToDeleteRevision> ToDelete, IWorkDistributor<BuildRequest> WorkDistributor, IActorRef ChangeTracker);

        public static IPreparedFeature New(IMongoCollection<AppData> apps, GridFSBucket files, RepositoryApi repository, DataTransferManager dataTransfer,
            IMongoCollection<ToDeleteRevision> toDelete, IWorkDistributor<BuildRequest> workDistributor, IActorRef changeTracker)
            => Feature.Create(() => new AppCommandProcessor(), _ => new AppCommandProcessorState(apps, files, repository, dataTransfer, toDelete, workDistributor, changeTracker));
        ...
        protected override void Config()
        {
                                                                           ...
                                                                           .Select(m =>
                                                                                   {
                                                                                       ...
                                                                                       var newData = new AppData(appName, -1, DateTime.UtcNow, DateTime.MinValue, targetRepo, projectName,
                                                                                                                 ImmutableList<AppFileInfo>.Empty);

                                                                                       state.Apps.InsertOne(newData);
            ...
            DirectCommandPhase1<PushVersionCommand>("PushVersion",
                                                    (reporter, input) => input.Select(m => new
                                                                                           {
                                                                                               App = m.State.Apps.AsQueryable().FirstOrDefault(ad => ad.Name == m.Event.AppName),
                                                                                               Data = m
                                                                                           })
                                                                              .ConditionalSelect()
                                                                              .ToResult<Unit>(s =>
                                                                                              {
                                                                                                  s.When(p => p.App == null,
                                                                                                         resultObs => resultObs
                                                                                                            .ToUnit(() => reporter.Compled(OperationResult.Failure(BuildErrorCodes.CommandAppNotFound))));

                                                                                                  s.When(p => p.App != null,
                                                                                                         resultObs => resultObs
                                                                                                            .ToUnit(m => BuildRequest
                                                                                                                        .SendWork(m.Data.State.WorkDistributor, reporter, m.App!,
                                                                                                                                  m.Data.State.Repository, BuildEnv.TempFiles.CreateFile())
                                                                                                                        .PipeTo(Self,
                                                                                                                                success: c => new ContinuePushNewVersion(OperationResult.Success(c), m.Data.Event, reporter),
                                                                                                                                failure: e => new ContinuePushNewVersion(OperationResult.Failure(e.Unwrap()?.Message ?? "Cancel"), m.Data.Event, reporter))))
                                                                                                      ;
        ...
        private void CommandPhase1<TCommand>(string name, Func<Reporter, IObservable<StatePair<TCommand, AppCommandProcessorState>>, 
                                                                                             ...
                                                                                             obs => obs.ToUnit(r =>
                                                                                                               {
                                                                                                                   ...
                                                                                                                   Log.Info("Command Phase 1 {Command} -- {Action}", typeof(TCommand).Name, 
                                                                                                                            msg!.GetType().Name);
        ...
        private void DirectCommandPhase1<TCommand>(string name, Func<Reporter, IObservable<StatePair<TCommand, AppCommandProcessorState>>, IObservable<Unit>> executor)
            where TCommand : ReporterCommandBase<DeploymentApi, TCommand>, IDeploymentCommand
            => Receive(name, executor);


        private void CommandPhase2<TContinue, TCommand, TResult>(string name,
                                                                 Func<IObservable<(TCommand Command, IOperationResult Result, Reporter Reporter, AppData? AppData, AppCommandProcessorState State)>, IObservable<TResult?>> executor)
            ...
            where TResult : class
        {
            ReceiveContinue<TContinue>(name, (reporter, observable) =>
                                             {
                                                 return executor(observable.SelectMany(async evt => (evt.Event.Command, evt.Event.Result, evt.Event.Reporter,
                                                                                           evt.State.Apps.AsQueryable().fi(ad => ad.Name == evt.Event.Command.AppName).FirstOrDefaultAsync(),
                                                                                           evt.State)))
        ...
        public static Task<AppData> QueryApp(IMongoCollection<AppData> collection, string name) 
            => collection.Find(Builders<AppData>.Filter.Where(ad => ad.Name == name)).FirstOrDefaultAsync();

        private static class Msg
        {
            public static NewMessage<TCommand> New<TCommand>(IReporterMessage? message, StatePair<TCommand, AppCommandProcessorState> pair)
                => new(message, pair.Event, pair.State);
        }

        private sealed record NewMessage<TCommand>(IReporterMessage? Message, TCommand Command, AppCommandProcessorState State);

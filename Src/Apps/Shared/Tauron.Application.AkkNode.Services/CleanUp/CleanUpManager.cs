...
{
    [PublicAPI]
    public sealed class CleanUpManager : IFeature<CleanUpManager.CleanUpManagerState>
    {
        ...
        public void Init(IFeatureActor<CleanUpManagerState> actor)
        {
            actor.Receive<InitCleanUp>(obs => obs.Where(m => !m.State.IsRunning)
                                                 .Select(init =>
                                                         {
                                                 ...
                                                 .Select(init => new { Database = init.State.Database.GetCollection<CleanUpTime>(init.State.CleanUpCollection), init.Timers })
                                                 .SelectMany(async data => new { Data = await data.Database.AsQueryable().FirstOrDefaultAsync(), data.Database, data.Timers })
                                                 .SelectMany(async data =>
                                                             {
                                                                 if (data.Data == null)
                                                                     await data.Database.InsertOneAsync(new CleanUpTime(default, TimeSpan.FromDays(7), DateTime.Now));
            ...
            actor.Receive<StartCleanUp>(obs => obs.Where(m => m.State.IsRunning)
                                                  .Select(data => CleanUpOperator.New(data.State.Database.GetCollection<CleanUpTime>(data.State.CleanUpCollection),
                                                                                         data.State.Revisions, data.State.Bucket))
                                                  .ForwardToActor(props => actor.Context.ActorOf(props)));
        }
    }
}
...
namespace ServiceManager.Server.AppCore.ServiceDeamon
{
    ...
    public sealed class ProcessServiceHostActor : ActorFeatureBase<ProcessServiceHostActor.State>
    {
            ...
            Receive<TryStart>(
                obs => obs.Synchronize()
                   ...
                   .ToResult<StatePair<TryStartResponse, State>>(
                        ...
                        {
                            b.When(
                                p => !p.State.Operator.Running,
                                o => from pair in o
                                     from result in TryStart(pair)
                                     select result);

                            b.When(
                                p => p.State.Operator.Running && p.State.Operator.CurrentUrl != p.Event.DatabaseUrl,
                                o => from pair in o
                                     from result in TryStart(pair)
                                     select result);
        ...
        private static void StartServices(
            string connectionstring, IHostEnvironment hostEnvironment, ActorSystem system,

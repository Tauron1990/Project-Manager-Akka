...
namespace AkkaTest.Feriertage
{
    public sealed record Laender(string Name, string Abkuerzung);

    public sealed record Feiertag(string Name, ImmutableList<Laender> Laender);

    public sealed record Feiertage(DateTime Datum, Feiertag Feiertag);

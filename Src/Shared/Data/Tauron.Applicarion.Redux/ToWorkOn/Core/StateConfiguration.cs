    ...
    where TState : class, new()
{
    private sealed class SimpleEffect : IEffect
    {
        ...
        public Effect<MultiState> Build()
            => Effects.CreateEffect<MultiState>(_run, _dispatch);
    }
    ...
    private sealed class SimpleStateEffect : IEffect
    {
        ...
        public Effect<MultiState> Build()
            => Effects.CreateEffect<MultiState>(store => _run(store.Select(state => state.GetState<TState>(_state))), _dispatch);
    }
    ...
    where TState : class, new()
{
    ...
    private sealed class RequestRegister<TAction> : IEffect
    {
        ...
        private IObservable<object> Process(ReduxStore<MultiState> store)
            => from input in store.ObserveAction<TAction, (TAction Action, MultiState State)>((action, applicationState) => (action, applicationState))
               select new StartRequest<TAction>(input.Action, _runRequest, _onSucess, _onFail);

        public Effect<MultiState> Build()
            => Effects.CreateEffect<MultiState>(Process, true);
    }
    ...
    private readonly List<On<MultiState>> _reducers;
    ...
    private void AddUpdater(IList<On<MultiState>> reducers, Guid stateId)
        => reducers.Add(
            Reducers.On<UpdateState, MultiState>(
    ...
    private static IEnumerable<On<MultiState>> CreateRequestFinisher<TAction>(Guid stateId)
        => Reducers.CreateSubReducers<MultiState, TState>(
    ...
    where TState : class, new() 
{
    ...
    public StateConfiguration(
        Guid stateId, IEventAggregator eventAggregator, Action<List<On<MultiState>>, List<IEffect>> config, 
    ...
    {
        MultiState Run(MultiState state, object action)

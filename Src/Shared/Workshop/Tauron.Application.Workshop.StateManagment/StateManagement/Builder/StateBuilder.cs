    ...
    where TData : class, IStateEntity
{
    ...
    private IStateInstance? CreateStateFactory(WorkspaceSuperviser superviser, IComponentContext? componentContext, ExtendedMutatingEngine<MutatingContext<TData>> dataEngine, IActionInvoker invoker)
    {
        ...
        if (componentContext != null)
            instance = componentContext.ResolveOptional(State, new TypedParameter(dataEngine.GetType(), dataEngine), new TypedParameter(typeof(IActionInvoker), invoker)) as IState;
    ...
    private IState? CreateStateFromConstructor(ExtendedMutatingEngine<MutatingContext<TData>> dataEngine, IActionInvoker invoker, IState? instance)
    {
        if (State is null) return null;

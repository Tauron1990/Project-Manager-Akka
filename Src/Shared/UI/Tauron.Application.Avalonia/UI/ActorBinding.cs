...
public class ActorBinding : UpdatableMarkupExtension
{
    ...
    protected override object ProvideValueInternal(IServiceProvider serviceProvider)
    {
        try
        {
            if(!TryGetTargetItems(serviceProvider, out AvaloniaObject? target, out _))
                return AvaloniaProperty.UnsetValue;

            if(!ControlBindLogic.FindDataContext(ElementMapper.Create(target).AsOption(), out DataContextPromise? model))
                return AvaloniaProperty.UnsetValue;
